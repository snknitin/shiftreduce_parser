...
class ParserState:
    ...
    def done_parsing(self):
        ...
        if len(self.stack)==1 and self.input_buffer[self.curr_input_buff_idx][0]==END_OF_INPUT_TOK:
            return True
        else:
            return False
    ...
    def _reduce(self, action):
        ...
        left = self.stack.pop()
        # Assigning head and dependent
        head,modifier = (left,right) if action==2 else (right, left)
        
        head_word,head_pos,head_emb = head
        modifier_word,modifier_pos,modifier_emb = modifier
        
        # Calling the combiner
        self.stack.append(StackEntry(head_word, head_pos, self.combiner(head_emb,modifier_emb)))
        return DepGraphEdge(head=(head_word,head_pos),modifier=(modifier_word,modifier_pos))
    ...
    def forward(self, sentence, actions=None):
        ...
        while parser_state.done_parsing() is False:
            ...
            if have_gold_actions:
                ...
                actions_done.append(current_action)
                if current_action==0:
                    parser_state.shift()
                else:
                    dep_graph.add(parser_state._reduce(current_action))
                       
                
            else:
                # First two valid actions need to be SHIFT
                current_action=utils.argmax(log_probs)
                
                if current_action==0:
                    if parser_state.input_buffer[parser_state.curr_input_buff_idx][0]!=END_OF_INPUT_TOK:
                        parser_state.shift()
                        actions_done.append(current_action)
                    else:
                        parser_state._reduce(Actions.action_to_ix["REDUCE_R"])
                        current_action = 2
                        actions_done.append(current_action)
...
def train(data, model, optimizer, verbose=True):
    ...
    if verbose:
        print("Number of instances: {%d}    Number of network actions: {%d}" %(instance_count, total_actions))
        print("Acc: {%.2f}  Loss: {%.3f}" %(float(correct_actions) / total_actions, tot_loss / instance_count))
