...
class BiLSTMWordEmbeddingLookup(nn.Module):
    ...
    def __init__(self, word_to_ix, word_embedding_dim, hidden_dim, num_layers, dropout):
        ...
        self.lstm = nn.LSTM(self.word_embedding_dim, self.hidden_dim/2, num_layers=self.num_layers, bidirectional=True,dropout=dropout)
    ...
    def forward(self, sentence):
        ...
        embeds=ag.Variable(torch.FloatTensor(len(inp), 1, self.word_embedding_dim))
...
class MLPCombinerNetwork(nn.Module):
    ...
    def __init__(self, embedding_dim):
        ...
        self.mlpl1 = nn.Linear(2*embedding_dim,embedding_dim) 
        self.tanh = nn.Tanh()
        self.mlpl2 = nn.Linear(embedding_dim, embedding_dim)
...
class LSTMCombinerNetwork(nn.Module):
    ...
    def __init__(self, embedding_dim, num_layers, dropout):
        ...
        self.mlplstm = nn.LSTM(2*self.embedding_dim, self.embedding_dim, num_layers=self.num_layers,dropout=dropout)
        #self.mlphl = nn.Linear(self.embedding_dim, self.embedding_dim)
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        x = utils.concat_and_flatten([head_embed, modifier_embed]).view(1, 1,-1)
...
class ActionChooserNetwork(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.afl1 = nn.Linear(input_dim,input_dim) 
        self.relu = nn.ReLU()
        self.afl2 = nn.Linear(input_dim, Actions.NUM_ACTIONS)
